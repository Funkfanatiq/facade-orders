...
def generate_torg12_xlsx(invoice, counterparty, config, template_path=None):
    ...
    # Рамки для печати (без 9–11 и 13–15 строки B–C, без столбцов 30–35)
    _apply_border(ws, 3, 2, 8, 3)
    _apply_border(ws, 12, 2, 12, 3)
    _apply_border(ws, 3, 4, 15, 29)
    _apply_border(ws, 2, 37, 17, 39)
    _apply_border(ws, 16, 11, 17, 22)
    # Таблица товаров: без границ в наименование (C–F) и код по ОКЕИ (L)
    _apply_border(ws, 20, 2, last_data_row, 2)    # B — №
    _apply_border(ws, 20, 7, last_data_row, 11)   # G–K — ед.изм
    _apply_border(ws, 20, 13, last_data_row, 13)  # M
